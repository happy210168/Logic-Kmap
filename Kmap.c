...
void Circle8(){
	int i,j,k,m,n=0;
	char binary[8][5]; //store the number in size 8 circle with binary term
	int num=0;	
	int count=0;
	...
	for(k=0;k<4;k++){           
		for(i=k;count<2;count++){
			for(j=0;j<4;j++){
				if(kmap[i][j]==1 && Circle[i][j]==0) flag2=1;
				if(kmap[i][j]==0){
					flag=1;
					break;
				}
	...
	for(k=0;k<4;k++){
		for(j=k;count<2;count++){
			for(i=0;i<4;i++){
				if(Circle[i][j]==0 && kmap[i][j]==1) flag2=1;
				if(kmap[i][j]==0){
					flag=1;
					break;
				}
...
void Circle4(){
	...
	for(k=0;k<4;k++){
			for(j=0;j<4;j++){
				if(Circle[k][j]==0 && kmap[k][j]==1) b2=1;
				if(kmap[k][j]==0){
					b=1;
					break;
				}
	...
	for(k=0;k<4;k++){
			for(i=0;i<4;i++){
				if(Circle[i][k]==0 && kmap[i][k]==1) b2=1;
				if(kmap[i][k]==0){
					b=1;
					break;
				}
...
int GetGroupTerm(char* input,int len,int term[]){
	...
	for(j=0;j<16;j++){
		...
		for(k=0;k<4;k++){
			total +=((c[j][k]-48) << (3-k));
//find group position and check whether this group is  primary implicant 
